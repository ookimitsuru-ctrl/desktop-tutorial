...
import com.bujo.app.ui.EntrySheetController
...
/**
 * マンスリーログ。紙のバレットジャーナルの見開き（左にカレンダー、右に一覧）を
 * 画面の形に合わせて再現する。横に余裕がある正方形の画面では左右に並べ、
 * 縦長のスマホでは上下に並べる。
 */
        ...
        if (window.useTwoPane) {
            Row(
                ...
                LazyColumn(
                    ...
                    monthlyEntries(entries, viewModel, controller)
                }
            }
        } else {
            LazyColumn(
...
private fun LazyListScope.monthlyEntries(
    entries: List<Entry>,
    viewModel: MonthlyViewModel,
    controller: EntrySheetController
) {
    item {
        SectionHeader(
            title = "今月のタスク・予定",
