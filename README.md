...
- 家畜 1 匹が 4 時間に 1 つ食べる (畑 1 つでだいたい 2 匹ぶんを養えます)
...
HUD 下部の **グラフ** ボタンで、**家畜の頭数・食料の量・空腹の続いている時間**の
移り変わりを折れ線グラフで確認できます。1 時間おきに記録し、直近 48 件
(だいたい直近 2 日ぶん) を表示します。記録はセーブデータに残るので、
アプリを開き直してもこれまでの記録は消えません
(ただし記録はアプリが開いていて時間が進んでいるときだけ取るので、
長く閉じていた間は「閉じる直前」と「開き直した瞬間」の間が飛びます)。
...
| 下 | 知らせ・建設中のものと残り時間 / **グラフ** ボタン / **つくる** ボタン |
...
- `PlanetState.CROP_PERIOD_MILLIS` — 畑ひとつが作物を1つ作る速さ (既定2時間ごと)。
  `EAT_PERIOD_MILLIS` はこれの2倍の値にしてあり、畑と生きものの必要な比率を保っています
- `PlanetState.HISTORY_SAMPLE_MILLIS` / `HISTORY_MAX_POINTS` — グラフ用の記録を取る間隔と残す件数
