...
テクスチャで塗ってあります (1文字ごとに違うブロック)。**「ほしにもどる」を押すと惑星に戻れます。**

### 待つ

**1 日に 2 回ほど、空から何かが降ってきます。** 降ってくる様子は種類ごとにまったく違います。
...
**作りはじめてからできあがるまでも実時間です。** **花とペットの種類は選べません** —
できあがったときにどんな花が咲くか、ヒツジかニワトリかブタかウシかが決まります。
花は 1 回の植え付けで何本かがまとまって咲きます。
...
**惑星が生まれて 1 週間経つと、ペットが作物を食べはじめます。**
畑をつくっておかないと食べるものが尽きて、ペットは元気が無くなってしまいます。

- 畑ひとつが、種まきから実るまで **1 週間ほど**かけて作物を 1 つ実らせる
- ペット 1 匹が 2 週間に 1 つ食べる (畑 1 つでだいたい 2 匹ぶんを養えます)

池・畑・ペットは、みんな惑星の**手前の面**に並びます。

### 宇宙船の襲来

**ペットがいると、3 日に 1 度くらいの割合で宇宙船が近づいてきます。** 結果は運まかせで 3 通り:

| 結果 | 何が起きるか |
| --- | --- |
| 攻撃される | 地表の **1/4 が荒れ地** になる (時間が経つと自然に元どおりになります) |
| 撤退する | 何も起きない。まれに鉱石・たね・氷を残していく |
| 追い払う | 宇宙船を乗り捨てて逃げていく。**レアアイテム**を手に入れる |

**花が多いほど「追い払う」確率が上がります**
(虫が苦手な宇宙人が、花に集まる虫に驚いて逃げていく、というイメージです)。
逆に花が少ないと攻撃を受けやすくなります。ペットがいなければ宇宙船は来ません。
...
- **宇宙船との駆け引き** — ペットを育てると宇宙船が寄ってくるようになります。
  花を増やして追い払うか、荒れ地になるのを覚悟するか、運まかせの駆け引きです。
...
- `PlanetState.ALIEN_INTERVAL_MILLIS` — ペットがいるときの宇宙船の襲来間隔
...
## デバッグ/デモ: 時間と演出の頻度を早回しする

**現在はデモ用に、1 日が 15 分になるよう早回ししてあります** (等倍の 96 倍速)。
リリースするときは以下を元に戻してください。
...
```kotlin
var SCALE: Float = 96f   // 96 なら 1日(24時間)がちょうど15分になる
```

太陽・月の動き、地殻変動、農業、飛来、建設の進み具合はすべてこの1箇所を基準にしているので、
数字を変えるだけで一括して速さが変わります (`1f` に戻すと実時間どおり)。
畑の作物は 1 週間かけて育つので、いまの設定 (1日=15分) では 1 サイクルおよそ 1 時間45分です。
...
こちらは `GameTime` と関係なく実時間で進むので、別に `core/Scene.kt` で調整します。
デモで見えやすいよう、本来より頻繁に出るようにしてあります。

| | いまの間隔 (デモ用) | 本来の間隔 |
| --- | --- | --- |
| UFO | 4〜10 秒おき | 40〜115 秒おき |
| よその惑星 | 20〜45 秒おき | 100〜260 秒おき |

`drawUfo()` / `drawFarPlanetDecor()` 内のコメントに両方の値を書いてあるので、
リリース時はそちらを参照して戻してください。
